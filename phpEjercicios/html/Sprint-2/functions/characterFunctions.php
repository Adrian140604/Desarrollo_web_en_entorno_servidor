...
function showFormButton($action,$character) {
    $button = '';
    $backButton = ""; // Initialize a variable for the back button
    $showFilmographyButton="";
    switch ($action) {
        case "editCharacter":
        ...
        default:
            ...
            $button = '<a href="./showCharacterList.php" class="btn btn-primary me-2">Volver</a>';
            $showFilmographyButton = '<a href="../characters/listFilmography.php?name=' . $character['nombre_persona'] . '" class="btn btn-primary me-2">Mostrar filmografía</a>';

    }

    // Printing the buttons
    print "<div class=\"row mb-3\">
            <div class=\"col-md-6 offset-md-3 d-flex justify-content-end\">$backButton $button $showFilmographyButton</div>
            </div>";
}
...
function showFilmography($nombre_persona, $connection) {
    $query = "SELECT p.titulo_p, t.tarea, p.ano_produccion FROM Trabajo t, Pelicula p WHERE nombre_persona = :nombre_persona AND t.cip = p.cip";
    ...
    if ($result) {
        foreach ($result as $character) {
            print "<tr>
                  <td class=\"align-middle\">{$character['titulo_p']}</td>
                  <td class=\"align-middle\">{$character['tarea']}</td>
                  <td class=\"align-middle\">{$character['ano_produccion']}</td>
                  <td class=\"text-end\"></td>

                  </tr>";
        }
        print "<a class=\"btn btn-outline-primary my-1\" href=\"./showCharacterList.php\">Volver</a> ";
    } else {
